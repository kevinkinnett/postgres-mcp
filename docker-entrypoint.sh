...
# Check if HTTP transports are specified and whether their bind hosts are already set
has_sse=false
has_sse_host=false
...
for arg in "${processed_args[@]}"; do
    if [[ "$arg" == "--transport" ]]; then
        # Check next argument for transport name
